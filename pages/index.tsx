...
type Good = typeof GOOD[number]; type Bad = typeof BAD[number];
...
const KEY_LEFT = new Set(["ArrowLeft", "a", "A"]); const KEY_RIGHT = new Set(["ArrowRight", "d", "D"]);
...
export default function Home() {
  ...
  // refs for stable RAF
  const timeRef = useRef(60); const powerRef = useRef(0); const livesRef = useRef(3); const xRef = useRef(0.5); const comboRef = useRef(0);
  ...
  // Avatar compose
  const [handle, setHandle] = useState(""); const [avatarUrl, setAvatarUrl] = useState("");
  const [glassesUrl] = useState("/billions-glasses.png"); // fixed asset
  const [composedUrl, setComposedUrl] = useState("");
  const [scale, setScale] = useState(1.0); const [offX, setOffX] = useState(0); const [offY, setOffY] = useState(0); const [rotate, setRotate] = useState(0);

  const dropsRef = useRef<Drop[]>([]); const floatiesRef = useRef<Floaty[]>([]); const particlesRef = useRef<Particle[]>([]);
  const keys = useRef({ L: false, R: false }); const last = useRef<number | null>(null);
  ...
  const fetchAvatar = async () => {
    const h = sanitizeHandle(handle); if (!h) { alert("Kullanıcı adı gir."); return; }
    const url = `/api/avatar?handle=${encodeURIComponent(h)}`; setAvatarUrl(url); await composeAvatar(url, glassesUrl);
  };
  const onLocalFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (!f) return; const url = URL.createObjectURL(f); setAvatarUrl(url); composeAvatar(url, glassesUrl);
  };
  ...
  useEffect(()=>{ if (avatarUrl) composeAvatar(undefined, glassesUrl); /* recompose on sliders */ }, [scale, offX, offY, rotate]);
  ...
  useEffect(()=>{
    ...
    const step = (t:number)=>{
      ...
      timeRef.current = timeRef.current>0 ? Math.max(0, timeRef.current-dt) : 0; setTimeLeft(timeRef.current);
      const speed=1.3; let nx=xRef.current; const dir=(keys.current.R?1:0)-(keys.current.L?1:0); nx=clamp01(nx+dir*speed*dt); xRef.current=nx; setX(nx);
      spawnAcc += dt; const spawnEvery=Math.max(0.4, 1.1-(60-Math.max(0,timeRef.current))*0.018);
      ...
      for(const d of dropsRef.current){ if(Math.abs(d.y-PY)<0.04 && Math.abs(d.x-nx)<0.08){ caught.push(d.id); if(d.good){ const mult=1+Math.floor(comboRef.current/5); const gain=5*mult; powerRef.current+=gain; setPower(powerRef.current); comboRef.current+=1; setCombo(comboRef.current); addFloaty(`+${gain} POWER ×${mult}`, d.x, PY); burst(d.x, PY, d.type); } else { livesRef.current=Math.max(0,livesRef.current-1); setLives(livesRef.current); comboRef.current=0; setCombo(0); addFloaty("-life", d.x, PY); burst(d.x, PY, d.type); } } }
      ...
      if(timeRef.current<=0 || livesRef.current<=0){ setRunning(false); last.current=null; setBest(b=>{ const n=Math.max(b,powerRef.current); localStorage?.setItem("billions_best", String(n)); return n; }); setScreen("gameover"); return; }
  ...
  const startGame=()=>{ reset(); setScreen("play"); setRunning(true); (document.activeElement as HTMLElement|null)?.blur(); };
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white flex items-center justify-center p-4 select-none">
      <div className="w-full max-w-5xl relative">
        ...
        <header className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm text-white/70 -mt-1">by <b>@traderibo123</b> • Build combos, catch valid items</p>
            </div>
          </div>
          <div className="flex gap-2">
            {screen === "play" && (<button onClick={()=>{reset(); setRunning(true);}} className="px-3 py-2 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 text-sm font-semibold">Restart</button>)}
          </div>
        </header>
        ...
        <footer className="mt-4 text-xs text-white/60 text-center">Fan-made visual demo • by <b>@traderibo123</b> • Tag <span className="font-semibold text-white">@billions_ntwk</span>.</footer>
      </div>
    </div>
  );
}
...
function IntroScreen(props: {
  ...
  const { handle, setHandle, fetchAvatar, onLocalFile, scale, setScale, offX, setOffX, offY, setOffY, rotate, setRotate, previewUrl, onStart } = props;
  return (
    <div className="rounded-[28px] border border-white/20 bg-white/5 p-4 shadow-[0_0_80px_#7c3aed55]">
      ...
      <p className="text-sm text-white/70 mb-4">Craft your hero: fetch your X avatar or upload one, then fit the <b>Billions glasses</b>. When ready, hit <b>Start</b>.</p>
        ...
        <div className="grid gap-3">
          <div className="grid md:grid-cols-3 gap-2 items-end">
            ...
            <div className="flex gap-2">
              ...
              <label className="px-3 py-2 rounded-2xl bg-white/10 border border-white/20 text-sm cursor-pointer">Upload<input type="file" accept="image/*" onChange={onLocalFile} className="hidden"/></label>
            </div>
          </div>
...
function Token({ d }: { d: Drop }) {
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 animate-wobble" style={{ left: `${d.x * 100}%`, top: `${d.y * 100}%` }} title={label}>
      <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-br ${grad} flex items-center justify-center shadow-[0_8px_30px_rgba(255,255,255,.18)] ring-2 ring-white/40 relative`}>
...
function HUD({ label, value }: { label: string; value: string }) {
  return (<div className="rounded-2xl bg-white/10 border border-white/20 shadow px-3 py-2"><div className="text-[10px] uppercase tracking-wide text-white/60">{label}</div><div className="text-lg font-bold leading-none">{value}</div></div>);
}
function Range({ label, value, min, max, step, onChange }: { label: string; value: number; min: number; max: number; step: number; onChange: (v:number)=>void }) {
  return (<label className="grid gap-1 text-xs text-white/70"><span>{label}: <b className="text-white">{typeof value === 'number' ? value.toFixed(2) : value}</b></span><input type="range" min={min} max={max} step={step} value={value} onChange={(e)=>onChange(parseFloat((e.target as HTMLInputElement).value))} className="appearance-none w-full h-2 rounded bg-white/10 accent-white" /></label>);
}
